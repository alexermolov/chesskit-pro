...
// Функция для копирования файлов локализации
function copyLocalizationFiles() {
  const sourceDir = path.join(__dirname, "..", "public", "locales");
  const targetDir = path.join(__dirname, "..", "out", "locales");

  if (!fs.existsSync(sourceDir)) {
    console.log("Папка public/locales не найдена");
    return;
  }
  ...
  // Копируем все файлы локализации
  const locales = fs.readdirSync(sourceDir);
  locales.forEach((locale) => {
    const sourceLocaleDir = path.join(sourceDir, locale);
    ...
    if (fs.statSync(sourceLocaleDir).isDirectory()) {
      ...
      files.forEach((file) => {
        if (file.endsWith(".json")) {
          const sourceFile = path.join(sourceLocaleDir, file);
          const targetFile = path.join(targetLocaleDir, file);
          fs.copyFileSync(sourceFile, targetFile);
          console.log(`Скопирован: locales/${locale}/${file}`);
        }
      });
    }
  });
...
function fixPaths() {
  ...
  jsFiles.forEach((filePath) => {
    ...
    content = content.replace(
      /\"\/([^\/][^"]*\.(woff2?|ttf|eot||jpg|jpeg|gif|webp|ico))\"/g,
      '"./$1"'
    );
    ...
    content = content.replace(/\"\/locales\//g, '"./locales/');
    ...
    content = content.replace(
      /("|\')\/([^\/"][^"']*\.(jpg|jpeg|gif|webp|ico|png|mp3|wav|ogg|js|wasm|nnue|ttf|woff2?|eot|json))\1/g,
      "$1./$2$1"
    );
